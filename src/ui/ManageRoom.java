...
public class ManageRoom extends javax.swing.JFrame {
   ...
   private final AppFacade facade = new AppFacade();
    ...
    public void s(){
        List<Object[]> rows = facade.listRooms();
...
dispose(); 
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        ActionResult<Void> result = facade.addRoom(
                txtno.getText(),
    ...
    if(evt.getClickCount()==2){
        String check=JOptionPane.showInputDialog(this,"If you want to Delete this Record Write here to Delete If you want to Update this Record then Write here update");
        DefaultTableModel dmodel=(DefaultTableModel) jTable1.getModel();
        int rows=jTable1.getSelectedRow();
        s2=(String) dmodel.getValueAt(rows,0);
        selectedStatus=(String) dmodel.getValueAt(rows,4);
        if(check.equalsIgnoreCase("delete")){
            ActionResult<Void> result = facade.deleteRoom(s2, selectedStatus);
            if (!result.isSuccess()) {
                if (result.hasMessage()) {
                    JOptionPane.showMessageDialog(this, result.getMessage());
                }
        ...
        if(check.equalsIgnoreCase("update")){
            ActionResult<Void> result = facade.canUpdateRoom(selectedStatus);
            if (!result.isSuccess()) {
                if (result.hasMessage()) {
                    JOptionPane.showMessageDialog(this, result.getMessage());
                }
    ...
    private void jButton3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton3ActionPerformed
        ActionResult<Void> result = facade.updateRoom(
                s2,
                jComboBox1.getItemAt(jComboBox1.getSelectedIndex()),
                jComboBox2.getItemAt(jComboBox2.getSelectedIndex()),
                txtprice.getText(),
